...
public class AppInitializer {
    ...
    public static void main(String[] args) {
        ...
        while (!exitState){
            /*for (int i = 0; i < initializePageQuestions.length; i++){
            System.out.println(initializePageQuestions[i]);
        }*/
    ...
    public static boolean register() {
        ...
        if (users[users.length - 1][0] != null) { //eka array ekakwath empty naththam (last user ge email eka not emptynm) (isEmpty advance nisa use kre na) (null -> String wala default eka nisa)
            System.out.println("User database is full!");
            return false; //JVM eka methanin pahalata yanne na
        }
        ...
        for (int x = 0; x < users.length; x++) {
            if (users[x][0] == null) { //email eka nullda kyla check krnne
                users[x][0] = email;
                users[x][1] = password;
                return true;
            } else {
